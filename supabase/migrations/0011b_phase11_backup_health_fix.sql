-- Phase 11 (fix) — Backup health snapshot
--
-- Fixes `public.get_backup_health()` which was returning 400 Bad Request
-- because it referenced `public.audit_log` (singular) but the actual table
-- created in 0001_init.sql:297 is `public.audit_logs` (plural).
--
-- This patch drops and recreates the function with the corrected table name.
-- It is otherwise identical to 0011_phase11_backup_health.sql.

set search_path = public;

drop function if exists public.get_backup_health(integer, integer);
...
begin
  ...
  with snapshot as (
    select * from (values
      ...
      ('audit_logs',      (select count(*) from public.audit_logs),      (select max(occurred_at) from public.audit_logs)),
